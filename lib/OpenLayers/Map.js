...
OpenLayers.Map.prototype = {
  ...
  // Options
    ...
    initialize: function (div, options) {
        ...
        this.div = div = MochiKit.DOM.getElement(div);

        // the viewPortDiv is the outermost div we modify
        var id = div.id + "_OpenLayers_ViewPort";
        this.viewPortDiv = MochiKit.DOM.DIV({'id':id,'style':{
        ...
        this.layerContainerDiv = MochiKit.DOM.DIV({'id':id,'style':{
            'position':'absolute',
            'margin':'0pt','padding':'0pt'}});
        ...
        MochiKit.Signal.connect(this, "resize", this.updateSize);
        ...
        MochiKit.Signal.connect(window, "onunload", this, this.destroy);
    },
  ...
  /********************************************************/         
    ...
    */    
    addLayer: function (layer) {
        layer.setMap(this);
        MochiKit.DOM.updateNodeAttributes(layer.div,{'style':{
            'overflow':'',
            'zIndex': this.Z_INDEX_BASE['Layer'] + this.layers.length}
            });
        if (layer.isFixed) {
        ...
        MochiKit.Signal.signal(this, "addlayer");
        ...
        if ( (this.baseLayer == null) && (layer.isBaseLayer()) ) {
            ...
            MochiKit.Signal.signal(this, "changebaselayer");
        }
    ...
    */    
    ...
    removeLayer: function(layer) {
        ...
        layer.map = null;
        MochiKit.Signal.signal(this, "removelayer");
    },
    ...
    */    
    addControl: function (control, px) {
        ...
        if (div) {
            MochiKit.DOM.updateNodeAttributes(div,{'style':{
                'zIndex': this.Z_INDEX_BASE['Control'] + this.controls.length}
                });
    ...
    */
    addPopup: function(popup) {
        ...
        if (popupDiv) {
            MochiKit.DOM.updateNodeAttributes(popupDiv,{'style':{
                'zIndex': this.Z_INDEX_BASE['Popup'] + this.popups.length}
                });
    ...
    */
    updateSize: function() {
        ...
        if (this.size.w == 0 && this.size.h == 0) {
            var dim = MochiKit.Style.getElementDimensions(this.div);
            this.size.w = dim.w;
            this.size.h = dim.h;
        }
        if (this.size.w == 0 && this.size.h == 0) {
    ...
    */
    setCenter: function (lonlat, zoom, minor) {
        ...
        if (zoomChanged || centerChanged) {
            ...
            if (!minor) { MochiKit.Signal.signal(this, "movestart"); }
            ...
            MochiKit.Signal.signal(this, "move");
    
            if (zoomChanged) { MochiKit.Signal.signal(this, "zoomend"); }
        }

        // even if nothing was done, we want to notify of this
        if (!minor) { MochiKit.Signal.signal(this, "moveend"); }
    },
  ...
  // Moved from Events
    ...
    */
    getMousePosition: function (clientXY) {
        if (!this.div.offsets) {
            this.div.offsets = MochiKit.Style.elementPosition(this.div);
        }
        return new OpenLayers.Pixel(
                        clientXY.x - this.div.offsets.x, 
                        clientXY.y - this.div.offsets.y); 
    },
